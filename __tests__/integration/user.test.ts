...
import {
  ...
  discordResponseGuildTwo,
  discordResponseGuildThree,
...
describe('User routes', () => {
  userService.getCurrentUserGuilds = jest
    .fn()
    .mockReturnValue([discordResponseGuildOne, discordResponseGuildTwo, discordResponseGuildThree]);
  ...
  describe('GET /api/v1/users/@me/guilds-with-admin-role', () => {
    ...
    test('should return 401 if access token is missing', async () => {
      await insertUsers([userOne]);
      await request(app).get('/api/v1/users/@me/guilds-with-admin-role').send().expect(httpStatus.UNAUTHORIZED);
    });
  });
