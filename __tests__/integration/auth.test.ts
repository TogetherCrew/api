...
describe('Auth routes', () => {
  describe('GET /api/v1/auth/try-now', () => {
    test('should return 302 when redirect correctly', async () => {
      await request(app).get('/api/v1/auth/try-now').send().expect(httpStatus.FOUND);
    });
  });
  ...
  describe('GET /api/v1/auth/try-now/callback', () => {
    ...
    test('should return 302 and successfully create user and guild', async () => {
      await request(app).get('/api/v1/auth/try-now/callback').query({ code: 'code' }).send().expect(httpStatus.FOUND);
    });
    ...
    test('should return 302 if code does not provided', async () => {
      await request(app).get('/api/v1/auth/try-now/callback').send().expect(httpStatus.FOUND);
    });
  });

  describe('GET /api/v1/auth/login', () => {
    test('should return 302 when redirect correctly', async () => {
      await request(app).get('/api/v1/auth/login').send().expect(httpStatus.FOUND);
    });
  });
  ...
  describe('GET /api/v1/auth/login/callback', () => {
    ...
    test('should return 302 and successfully create user and guild', async () => {
      ...
      await insertGuilds([guildOne]);
      await request(app).get('/api/v1/auth/login/callback').query({ code: 'code' }).send().expect(httpStatus.FOUND);
    });
    ...
    test('should return 302 if code does not provided', async () => {
      await request(app).get('/api/v1/auth/login/callback').send().expect(httpStatus.FOUND);
    });
  });
