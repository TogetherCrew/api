...
describe("Community routes", () => {
    const announcementOne = generatePublicDiscordAnnouncement(
        communityOne._id,
        platformOne._id,
        [discordChannel4.channelId, discordChannel3.channelId],
    );
    const announcementTwo = generatePrivateUserDiscordAnnouncement(
        communityOne._id,
        platformOne._id,
        [discordGuildMember1.discordId, discordGuildMember2.discordId],
    );
    const announcementThree = generatePrivateRoleDiscordAnnouncement(
        communityThree._id,
        platformOne._id,
        [userOne._id, userTwo._id],
    );

    let connection: Connection;

    beforeAll(async () => {
        connection = await DatabaseManager.getInstance().getTenantDb(
    ...
    describe("POST api/v1/announcements", () => {
        ...
        test("should return 201 and successfully create a new draft announcement if data is ok", async () => {
            ...
            await insertChannels(
                [discordChannel4, discordChannel3],
                connection,
            );
            await insertGuildMembers(
                [discordGuildMember1, discordGuildMember2],
                connection,
            );
