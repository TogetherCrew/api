...
async function enhanceAnnouncementData(announcementData: Array<any>) {
  ...
  for (const announcementD of announcementData) {
    const enhancedOptions = await announcementService.enhanceAnnouncementDataOption(
      announcementD.platform,
      announcementD.options,
    );
    const type = announcementService.generateAnnouncementType(announcementD);
    enhancedData.push({
      platform: announcementD.platform,
      template: announcementD.template,
...
const updateAnnouncement = catchAsync(async function (req: IAuthRequest, res: Response) {
  ...
  if (announcement.draft == true) {
    if (draft == true || draft == undefined) {
      newAnnouncement = await announcementService.findOneAnnouncementAndUpdate(
        { _id: announcementId },
        announcementData,
      );
    }
    if (draft == false) {
      newAnnouncement = await announcementService.updateAnnouncementAndAddJob(
        announcementId,
        announcement,
        announcementData,
      );
  ...
  if (announcement.draft == false) {
    if (draft == false || draft == undefined) {
      if (scheduledAt == undefined || announcement.scheduledAt.toISOString() == scheduledAt.toISOString()) {
        ...
        newAnnouncement = await announcementService.updateAndRescheduleAnnouncement(
          announcementId,
          announcement,
          announcementData,
        );
    ...
    if (draft == true) {
      newAnnouncement = await announcementService.updateAnnouncementAndRemoveJob(
        announcementId,
        announcement,
        announcementData,
      );
...
const getAnnouncements = catchAsync(async function (req: IAuthRequest, res: Response) {
  ...
  paginatedAnnouncement.results = await Promise.all(
    paginatedAnnouncement.results.map(
      async (announcement: any) => await getAnnouncementFieldsToReturn(announcement as IAnnouncement),
    ),
  );
