...
const disconnectTwitter = catchAsync(async function (req: IAuthRequest, res: Response) {
  ...
  await Token.deleteMany({
    user: user.discordId,
    type: { $in: [tokenTypes.TWITTER_ACCESS, tokenTypes.TWITTER_REFRESH] },
  });
...
const refreshTwitter = catchAsync(async function (req: IAuthRequest, res: Response) {
  ...
  if (!user?.twitterUsername) {
    throw new ApiError(
      400,
      'Oops, It seems you have not connected your `Twitter` account, try setup `Twitter` and try again!',
    );
  }
...
const engagementMetrics = catchAsync(async function (req: IAuthRequest, res: Response<TwitterEngagementResponse>) {
  ...
  const replyQuoteMentionUsers = new Set([
    ...repliesInteractionUsers,
    ...quotesInteractionUsers,
    ...mentionsInteractionUsers,
  ]);
  for (const userId of Array.from(replyQuoteMentionUsers)) {
    ...
    if (replyInteractionNumber + quoteInteractionNumber + mentionInteractionNumber < 3) hqla++;
    else hqhe++;
  }
  ...
  for (const userId of Array.from(retweetLikeUsers)) {
    ...
    if (retweetInteractionNumber + likeInteractionNumber < 3) lqla++;
    else lqhe++;
  }
