...
const getChannels = catchAsync(async function (req: IAuthRequest, res: Response) {
  ...
  for (let i = 0; i < channels.length; i++) {
    const canReadMessageHistoryAndViewChannel = await channelService.checkReadMessageHistoryAndViewChannelpPermissions(
      connection,
      channels[i],
    );
...
const getSelectedChannels = catchAsync(async function (req: IAuthRequest, res: Response) {
  ...
  if (guild.selectedChannels && guild.selectedChannels.length > 0) {
    ...
    sortedChannels = sortedChannels
      .filter((category) => {
        const selectedSubChannels = category.subChannels.filter(
          (channel: IChannel) => guild.selectedChannels?.some((selected) => selected.channelId === channel.channelId),
        );
      ...
      .map((category) => {
        return {
          ...
          subChannels: category.subChannels.filter(
            (channel: IChannel) => guild.selectedChannels?.some((selected) => selected.channelId === channel.channelId),
          ),
        };
      });
...
const connectGuild = catchAsync(async function (req: IAuthRequest, res: Response) {
  res.redirect(
    `https://discord.com/api/oauth2/authorize?client_id=${config.discord.clientId}&redirect_uri=${
      config.discord.callbackURI.connectGuild
    }&response_type=code&scope=${scopes.connectGuild}&permissions=${
    ...
    if (user) {
      ...
      if (guild) {
        ...
        await guildService.updateGuild(
          { guildId: discordOathCallback.guild.id, user: discordUser.id },
          { isDisconnected: false },
        );
      } else {
        statusCode = 701;
