...
try {
    ...
    logger.info({ url: config.neo4j.url, user: config.neo4j.user, password: config.neo4j.password }, `Connected to Neo4j!`);
} catch (error) {
    logger.fatal({ url: config.neo4j.url, user: config.neo4j.user, password: config.neo4j.password }, `Failed to connect to Neo4j!`);
}
